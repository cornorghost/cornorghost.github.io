...
function init() {
  document.getElementById('highest scores').innerText=myStorage.highest;
  snakeBody=[];
  length=0;
  for(var i=0;i<3;i++){
...
function putFood() {
  ...
  food={x:foodX,y:foodY,colo:'#f40'};
}

function isGetfood(node) {
  if(count>localStorage.highest) {
  ...
  if(node.x==food.x && node.y==food.y){
    ...
    document.getElementById('scores').innerHTML=(++count);
  }
}

function isGameover() {
  if(snakeBody[0].x<0 || snakeBody[0].x>=cvsGridX || snakeBody[0].y<0 || snakeBody[0].y>=cvsGridY) return 1;
  for(var i=snakeBody.length-1;i>0;i--){
    if(snakeBody[i].x==snakeBody[0].x && snakeBody[i].y==snakeBody[0].y) return 1;
  }
}

function changePicFirst(whichPic) {
  var src=whichPic.getAttribute('src');
  var alt=whichPic.getAttribute('alt');
  whichPic.setAttribute("src",alt);
  whichPic.setAttribute("alt",src);
}

function changePicBack(whichPic) {
  var src=whichPic.getAttribute('src');
  var alt=whichPic.getAttribute('alt');
  whichPic.setAttribute("src",alt);
  whichPic.setAttribute("alt",src);
